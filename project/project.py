...
chapters = [x for x in chapters if x is not None]
#print(chapters)
...
def get_paper(hid):
    ...
    tmplog = "{home}/{hid}/{paper}/tmp.log".format(paper=kind, home=d["home"], hid=hid)    
...
for chapter in order:
    ...
    for hid in hids:
        # print (hid)
        if data[hid] is not None  and 'exclude' not in data[hid] and data[hid]['chapter'] == chapter:
            try:
                get_paper(hid)
            except:
                pass
